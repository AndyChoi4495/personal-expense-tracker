  ...
  const handleSend = async (textToSend) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...
        {
          role: 'ai',
          content: 'Error: Could not connect to the AI advisor.',
        },
      ]);
    } finally {
      setIsLoading(false);
    ...
    <div className="fixed bottom-8 right-8 z-[9999]">
      {isOpen ? (
        <div className="w-80 sm:w-96 bg-white rounded-[32px] shadow-2xl border border-gray-100 flex flex-col overflow-hidden animate-in fade-in zoom-in duration-300">
          ...
          <div className="h-96 overflow-y-auto p-6 space-y-4 bg-gray-50 flex flex-col">
            {messages.length === 0 && (
          ...
          <div className="p-4 bg-white border-t border-gray-100 flex gap-2">
            <input
              className="flex-1 bg-gray-100 rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-indigo-500 transition-all text-black font-medium"
        ...
        >
          ...
          />
          <span className="font-bold uppercase text-[10px] tracking-[0.2em] text-black">
            Ask AI Advisor
          </span>
        </button>
      )}
